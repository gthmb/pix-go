...
func (playerGame PlayerGame) render() (rendered Rendered) {
	rendered = Rendered{
		PlayerGame: playerGame,
		Links:      make(map[string]string),
	}
	rendered.Links["self"] = fmt.Sprintf("%s/playergames/%s", util.Host, playerGame.ID)
	rendered.Links["start"] = fmt.Sprintf("%s/playergames/%s/draw", util.Host, playerGame.ID)
	return
}
...
func HandleList(w http.ResponseWriter, r *http.Request) {
	...
	switch r.Method {
	case "GET":
		util.WriteJSONResponse(w, PlayerGameMap.render())
	case "POST":
		...
		util.WriteJSONResponse(w, playerGame.render())
	}
}
